...
import TransactionSuccesfullyModal from "./TransactionSuccesfullyModal"
import { PATHS } from "../../routes/routes"
const PRICE_PER_PET_PET_NIGHT = 15000
const COMMISSION = 1000

const HireSummaryAside = () => {
  const [showTransactionSuccesfullyModal, setShowTransactionSuccesfullyModal] = useState(false)
  ...
  return (
    <>
      {
        showTransactionSuccesfullyModal && <TransactionSuccesfullyModal pathGoTo={PATHS.home} />
      }
      <div className="w-full sm:min-w-[370px] flex-1 min-[1100px]:px-6">
        <aside className="min-[1100px]:sticky min-[1100px]:top-6 w-full flex flex-col gap-6 border border-gray-light rounded-xl py-4 px-6">
          ...
          <button onClick={() => setShowTransactionSuccesfullyModal(true)} className="bg-btn text-white font-semibold rounded-lg p-2 w-full">Contratar</button>
        </aside>
      </div>
    </>
  )
